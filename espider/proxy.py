...
class Proxy(object):
    ...
    def __init__(self, startUrl = None):
        ...
        path, file = os.path.split(config.configs.proxy.srcname)
        if not os.path.exists(path):
            os.makedirs(path)
        if startUrl == None:
            self.startUrl = 'http://www.baidu.com'
        if config.configs.proxy.rescrab:
            Logger.info('rescrab proxylist...')
            self.getFreeProxy()
        if config.configs.proxy.retest:
            Logger.info('retest proxy list...')
            self.testProxy()
        if not os.path.exists(config.configs.proxy.srcname):
    ...
    def loadProxy(self):
        data = readLinesFile(config.configs.proxy.srcname)
        if data == None:
    ...
    def getFreeProxy(self):
        ...
        proxyList = []
        if config.configs.proxy.proxysrc == 1:
            for i in range(config.configs.proxy.srcpage):
        ...
        if config.configs.proxy.proxysrc == 2:
            for i in range(config.configs.proxy.srcpage):
                Logger.info('get page %s...' %(i + 1))
        ...
        writeLinesFile(config.configs.proxy.srcname, dataset)
    ...
    def testProxy(self):
        ...
        for k,v in config.configs.urlrequest.items():
            req.add_header(k,v)
        Logger.info('test proxy list in %s' % config.configs.proxy.srcname)
        data = readLinesFile(config.configs.proxy.srcname)
        time.clock()
        proxyList = []
        for i in range(len(data)):
            ...
            try:
                begin = time.clock()
                openner.open(req, timeout=config.configs.proxy.timeout)
    ...
    def getReqWithProxy(self, req, **kw):
        ...
        for i in range(len(self.proxyList)):
            try:
                ...
                response.read(0)
                flag = True
                if config.configs.proxy.mode == 2:
                    self.nextPrxoy()
                break
            except Exception:
                self.nextPrxoy()
